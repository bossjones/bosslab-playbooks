#!/usr/bin/env bash
# SOURCE: https://github.com/kubernetes/kubernetes/issues/24873
    ...
    if (( $i < 1 )); then
        ...
        if [[ "$namespace" != "NAMESPACE" ]]; then
            ...
            if [[ $kind != 'Secret' || $yaml != *"type: kubernetes.io/service-account-token"* ]]; then
                ...
                kubectl get $kind -o=yaml --export $name -n $namespace > ${_DIR}/../dump/$namespace/$kind.$name.yaml
            fi
        fi
    fi
done
